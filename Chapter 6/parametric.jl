...
Point{AbstractString} #> Point{AbstractString} (constructor with 1 method)

issubtype(Point{Int64}, Point) #> true
issubtype(Point{AbstractString}, Point) #> true
issubtype(Point{Float64}, Point{Real}) #> false

p = Point{Int64}(2, 5) #> Point{Int64}(2,5)
p = Point(2, 5) #> Point{Int64}(2,5)
p = Point("London", "Great-Britain") #> Point{AbstractString}("London", "Great-Britain")
...
function vecfloat{T <: AbstractFloat}(x::Vector{T}) 
# code   
end